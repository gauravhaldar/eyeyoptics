...
export default function WomenEyeglassesPage() {
  ...
  const filters = {
    ...
    "Frame Shape": ["Cat Eye", "Round", "Rectangle", "Square"],
    "Frame Color": ["Black", "Pink", "Brown", "Transparent", "Gold"],
    "Frame Material": ["Metal", "Plastic", "TR90", "Titanium"],
    Brands: ["Ray-Ban", "Vogue", "Fastrack", "Titan"],
    Collections: ["Trendy", "Casual", "Professional", "Premium"],
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        setLoading(true);
        setError(null);
        const res = await fetch("http://localhost:8000/api/products?women=true");
        if (!res.ok) {
  ...
  const filteredProducts = useMemo(() => {
    let filtered = Array.isArray(products) ? products.filter(product => product.women) : [];
  ...
  return (
    <div className="w-full bg-gradient-to-b from-pink-50 via-pink-100 to-pink-200 min-h-screen">
      {/* Top Banner */}
      <motion.div
      ...
      <div className="flex flex-col md:flex-row">
        {/* Sidebar */}
        <motion.aside
          className="w-full md:w-72 bg-gradient-to-b from-pink-100 to-pink-200 p-5 space-y-6 md:min-h-screen shadow-inner"
          initial="hidden"
          animate="show"
        ...
        <main className="flex-1 p-6">
          ...
          <div className="flex items-center justify-between mb-6 border-b border-gray-300 pb-3">
            ...
            <select
              ...
              className="bg-gradient-to-r from-pink-100 to-pink-200 border border-gray-300 rounded px-3 py-2 shadow-sm focus:outline-none focus:ring-2 focus:ring-pink-400 transition"
            >
          ...
          <motion.div
            ...
            variants={containerVariants}
          >
            {loading && <p className="text-center col-span-full text-gray-700">Loading women's eyeglasses...</p>}
            {error && <p className="text-center col-span-full text-red-500">{error}</p>}
            {!loading && !error && filteredProducts.length === 0 && (
              <p className="text-center col-span-full text-gray-700">No women's eyeglasses found.</p>
            )}
