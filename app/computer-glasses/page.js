  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        setLoading(true);
        setError(null);
        const res = await fetch("http://localhost:8000/api/products?category=Computer Glasses", {
          method: "GET",
        });
        if (!res.ok) {
    ...
    <div className="w-full bg-gradient-to-b from-gray-50 via-gray-100 to-gray-200 min-h-screen">
      ...
      <div className="flex flex-col md:flex-row">
        ...
        <main className="flex-1 p-6">
          ...
          {/* Products Grid */}
          {loading ? (
            <div className="col-span-full text-center py-10 text-lg text-gray-600">Loading computer glasses...</div>
          ) : error ? (
            <div className="col-span-full text-center py-10 text-lg text-red-500">{error}</div>
          ) : filteredProducts.length === 0 ? (
            <div className="col-span-full text-center py-10 text-lg text-gray-600">No computer glasses found.</div>
          ) : (
            <motion.div
